...
import { Box, Text, Image, Icon, FlatList, ScrollView, useToast, HStack, Pressable } from 'native-base'
...
const ProcedureBookingPayment = ({navigation, route}) => {
    ...
    useEffect(() => {
        if(item){
            setData(item?.procedure_id.map(pro => {
                return {
                    ...pro,
                    selected: true
                }
    ...
    const getTimeAvailability = async(data) => {
        ...
        await customAxios.post(`patient/procedure/time`,data)
            .then(async response => {
                ...
                if(response?.data?.data?.length === 0){
                    toast.show({
                        title: 'Info',
                        description: 'No Time slot available',
    ...
    const Submit = async() => {
        let datas = {
            ...
            procedure_id: data?.map(proId => proId?.id),
            amount: price,
        ...
        await customAxios.post(`patient/procedure/booking`, datas)
            .then(async response => {
                let booking = response?.data?.data;
                var options = {
                    description: 'Procedure Booking',
    ...
    const paymentSuccess = async(bookingdata) => {
        await customAxios.post(`patient/procedure/booking/update`, bookingdata)
        .then(async response => {
            dispatch({
                type: LOADING,
                payload: false
            ...
            //navigation.pop(4)
            navigation.navigate("ProcedureBooking")
        }).catch(async error => {
            toast.show({
                title: 'Error',
                description: error,
  ...
  return (
    <CustomBackground>
        <Box px={5} mt={5} >
            <CommonHeading label={'Book Procedure'} goBack={()=>navigation.goBack()}/>
        </Box>
        <Box borderTopRadius={30} bg='#fff' mt={5} flex={1} px={5}>
            ...
            <FlatList 
                ...
                ListFooterComponent={()=> <>
                    <CommonActionButton 
                        onPress={selectedTime ? Submit : errorToast}
                        bg={selectedTime ? {
                            linearGradient: {
...
export default ProcedureBookingPayment
